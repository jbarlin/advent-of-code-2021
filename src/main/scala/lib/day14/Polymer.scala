...
final class Polymer(elemPairs: Map[(Char, Char), Long], last: Char) {
    ...
    lazy val elemCounts: Iterable[Long] = elemPairs
    ...
    def mostCommonCount: Long = elemCounts.max

    def leastCommonCount: Long = elemCounts.min
}
