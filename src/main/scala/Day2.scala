...
object Day2 {
  def parseInput(): List[SubMovement] = {
      Source.fromResource("day2.txt").getLines.toList.map((s: String) => {
  ...
  def partOne(input: List[SubMovement]): String = {
      val finalCoord: Coords = input.foldLeft(new Coords(0,0))((coord, movement) => movement.addToCoords(coord))
            ...
            .foldLeft
                ...
                ((posn, movement) => {
                    movement match {
                        ...
                        case ForwardSubMovement(x: Int) => (movement.addToCoords(posn._1) + new Coords(0, x * posn._2), posn._2)
